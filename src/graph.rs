...
impl Graph {
  ...
  pub fn remove_vertex(&mut self, index: i32) {
    //Remove linked edges 
    if let Some(vertex_index) = self.vertices.iter().position(|x| x == &Vertex::new(index, 0)) {
  ...
  pub fn add_edge(&mut self, vertex_a: i32, vertex_b: i32, weight: i32) {
    if !self.get_vertices().contains(&vertex_a) || !self.get_vertices().contains(&vertex_b){
      ...
      self.edges.contains(&Edge::new(vertex_a, vertex_b, weight)) {
      println!("Graph already contains edge with pair {:?}.", (vertex_a, vertex_b));
    } else {
  ...
  pub fn get_vertex_value(&mut self, index: i32) -> i32 {
    if let Some(exists) = self.vertices.iter().position(|x| x == &Vertex::new(index, 0)){
